...
class VoiceChatAreaState extends ConsumerState<VoiceChatArea> {
  ...
  void _send() {
    ...
    final myName = me.name ?? '匿名用户';
    final old = ref.read(voiceChatProvider(widget.roomId));
    ref.read(voiceChatProvider(widget.roomId).notifier).state = [
      ...old,
      VoiceChatMessage(uid: myId, userName: myName, content: text),
    ];
    _controller.clear();
    _scrollToBottom();
  }
      ...
      _ManageAction(
        ...
        onTap: () {
          ref
              ...
              .setMusic(!room.musicOn);
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(room.musicOn ? '音乐已关闭' : '音乐已开启')),
          );
        },
      ),
      _ManageAction(
        ...
        onTap: () {
          ref
              .read(voiceRoomProvider(widget.roomId).notifier)
              .setRecording(!room.recordingOn);
          Navigator.pop(context);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(room.recordingOn ? '录播已关闭' : '录播已开启')),
          );
        },
      ),
      _ManageAction(
      ...
      _ManageAction(
        ...
        onTap: () {
          ...
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('房间分享链接已复制（演示）')));
        },
      ),
      _ManageAction(
        ...
        label: '清空消息',
        onTap: () {
          ref.read(voiceChatProvider(widget.roomId).notifier).state = const [
            VoiceChatMessage(
              uid: 'sync',
              userName: '系统提示',
              content: '聊天已由房主清空',
                  ...
                  child: muted.isEmpty
                      ...
                      : ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final title = seat.name?.isNotEmpty == true
                                ? seat.name!
                                : (seat.uid ?? '未知用户');
                            final sub =
                                'UID: ${seat.uid ?? "未知"} · ${seat.index}号位';
                            return ListTile(
                              title: Text(
                                title,
                              ...
                              trailing: TextButton(
                                onPressed: () {
                                  ref
                                      ...
                                      .setMuted(seat.index, false);
                                  Navigator.pop(context);
                                  ScaffoldMessenger.of(
                                    this.context,
                                  ).showSnackBar(
                                    SnackBar(content: Text('已解除 $title 的禁言')),
                                  );
                                },
                      ...
                      children: [
                        TextSpan(
                          ...
                          style: const TextStyle(
                            color: Color.fromARGB(255, 97, 181, 255),
                            fontWeight: FontWeight.bold,
                          ),
                        ),
                        TextSpan(
              ...
              if (widget.canShowGift)
                GestureDetector(
                  onTap: () {
                    ScaffoldMessenger.of(
                      context,
                    ).showSnackBar(const SnackBar(content: Text('打开送礼面板')));
                  },
                  child: const Icon(
                    Icons.card_giftcard,
