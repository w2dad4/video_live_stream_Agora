...
class PermissionDialog extends StatelessWidget {
  ...
  const PermissionDialog({super.key, required this.title, required this.message, required this.icon, required this.iconColor});
...
class _HomePermissionRequestState extends State<HomePermissionRequest> {
  ...
  Future<void> _requestPermissionsSequentially() async {
    ...
    // 按优先级顺序请求权限：camera -> mic -> location
    final permissions = [_PermissionItem(permission: Permission.camera, title: '开启摄像头权限', message: '需要摄像头权限才能进行直播和连麦互动', icon: Icons.videocam, iconColor: Colors.red), _PermissionItem(permission: Permission.microphone, title: '开启麦克风权限', message: '需要麦克风权限才能进行语音直播和聊天', icon: Icons.mic, iconColor: Colors.blue)];
    ...
    for (var item in permissions) {
      ...
      if (status.isDenied) {
        ...
        final result = await showDialog<bool>(
          ...
          builder: (context) => PermissionDialog(title: item.title, message: item.message, icon: item.icon, iconColor: item.iconColor),
        );
  ...
  Future<void> _handleNotificationPermission() async {
    ...
    final result = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (context) => const PermissionDialog(title: '开启通知权限', message: '开启通知可以及时收到直播间消息和互动提醒，比如消息提醒、主播开播提醒等', icon: Icons.notifications, iconColor: Colors.orange),
    );
...
class _PermissionItem {
  ...
  _PermissionItem({required this.permission, required this.title, required this.message, required this.icon, required this.iconColor});
}
